...
export function LoadingExperience({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const openTimer = window.setTimeout(() => setOpen(true), reduceMotion ? 150 : 1750);
    const doneTimer = window.setTimeout(onComplete, reduceMotion ? 500 : 3300);
    return () => {
  ...
  const panelTransition = { duration: reduceMotion ? .15 : 1.45, ease: [0.76, 0, 0.24, 1] };
  ...
  return (
    <motion.div className="loader" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .45 }} aria-label="Opening Aurelis">
      <motion.div className="loader__seam" aria-hidden="true" animate={{ opacity: open ? 0 : 1 }} transition={{ duration: .35 }}>
        <motion.span className="loader__seam-line loader__seam-line--top" initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} transition={{ duration: reduceMotion ? .1 : 1.25, ease: [0.76, 0, 0.24, 1] }} />
        <motion.span className="loader__seam-line loader__seam-line--bottom" initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} transition={{ duration: reduceMotion ? .1 : 1.25, ease: [0.76, 0, 0.24, 1] }} />
      </motion.div>
